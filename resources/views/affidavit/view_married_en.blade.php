        ...
        <div class="body_content">
            <div class="panel_container">
                ...
                <div class="panel_body">
                    ...
                    <div class="enbn_btn">
                        <a href="{{route('admin.affidavit.view_married', $info->id)}}" class="view">
                    ...
                    <div class="affidavit">
                        ...
                        <div class="print_header_info">
                            ...
                            <div class="title">
                                ...
                                <h2><strong>Married Certificate</strong></h2>
                        ...
                        <div class="affidavit_body">
                            <div class="row">
                                ...
                                <div class="col col-sm-4">
                                    <p class="text-right">
                                        ...
                                        {{ numberFilter($info->marriage_date,'en') }}
                                    </p>
                                </div>
                            </div>
                            ...
                            Son of <span>{{ $info->father_name_en }}</span> & <span>{{ $info->mother_name_en }}</span>
                            OF Village: <span>{{ $info->village_en }}</span>,
                            Post Office: <span>{{ $info->post_office_en }}</span>,
                            Upazila: <span>{{ $upazilas_name->name }}</span>,
                            District: <span>{{ $districts_name->name }}</span>
                            Date of Birth <span>{{ numberFilter($info->dob, 'en') }}</span>
                            married with <span>{{ $info->wife_name_en }}</span>
                            Daughter of <span>{{ $info->wife_father_name_en }}</span> & <span>{{ $info->wife_mother_name_en }}</span>
                            OF Village: <span>{{ $info->wife_village_en }}</span>,
                            Post Office: <span>{{ $info->wife_post_office_en }}</span>,
                            Upazila: <span>{{ $wifeUpazila->name }}</span>,
                            District: <span>{{ $wifeDistrict->name }}</span>
                            Date of Birth <span>{{ numberFilter($info->wife_dob, 'en') }}</span>.
                            The marriage was solemnized on <span>{{ numberFilter($info->marriage_date,'en') }}</span>
                            and registered in my office on <span>{{ numberFilter($info->ragi_date, 'en') }}</span>
                            being registration serial no <span>{{ numberFilter($info->ragi_serial_no, 'en') }}</span> in
                            page no <span>{{ numberFilter($info->ragi_page_no, 'en') }}</span>
                            of Column no <span>{{ numberFilter($info->ragi_column_no, 'en') }}</span>
                            in the year <span>{{ numberFilter($info->ragi_year, 'en') }}</span>
                            in <span>{{ $info->regi_address_en }}</span>

                            <br />
                            <br />
                            I wish them all the best and prosperity. <br />
                            Issue Date <span>{{ numberFilter($info->created,'en') }}</span>
                            </p>
